...
from .wflow_lib_federico import *
import time
import calendar
import datetime
...
class wf_DynamicFramework(frameworkBase.FrameworkBase):
    ...
    def __init__(
        ...
        self.exchangeitems = wf_exchangeVariables()
        self.setQuiet(True)
        self.reinit = 0
    ...
    def createRunId(
        ...
        if hasattr(self._userModel(), "config_reservoir_file"):
            config_reservoir_file = self._userModel().config_reservoir_file
        if not os.path.isdir(caseName + "/" + runId):
            ...
            os.makedirs(caseName + "/" + runId + "/reservoirs/")
        else:
        ...
        if hasattr(self._userModel(), "config_reservoir_file"):
            try:
                shutil.copy(
                    caseName + "/" + config_reservoir_file, caseName + "/" + runId + "/runinfo"
                    #caseName + "/reservoirs/" + config_reservoir_file, caseName + "/" + runId + "/runinfo"
                )
                print("          RESERVOIR FILE COPIED CORRECTLY!!!                 ")
            except:
                print("Cannot find config reservoir file: " + caseName + "/" + config_reservoir_file)
        ...
        if hasattr(self._userModel(), "config_reservoir_file"):
            self._userModel().reservoir_config = self.iniFileSetUp(caseName, runId, config_reservoir_file)
            print("                  RESERVOIR CONFIG INITIALISED                  ")
        else:
            print("                  SOMETHING BAD IN FRAMEWORK                  ")
    ...
    def setupFramework(self):
        ...
        # Now finally set the start and end time. First check if set in ini otherwise check if the ini defines
        # a runinfo file
        ...
        if hasattr(self._userModel(), "config_reservoir_file"):
            meta["wflow_res_ini"] = self._userModel().config_reservoir_file
        ...
        if hasattr(self._userModel(), "config_reservoir_file"):
            self._userModel().reservoir_IDs = pd.read_table(os.path.join(self._userModel().Dir, self._userModel().reservoir_txtfile), squeeze=True)
            all_reservoirLocs_df = pd.read_table(os.path.join(self._userModel().Dir, self._userModel().reservoir_coords), index_col='damID')
            self._userModel().reservoirLocs_df = all_reservoirLocs_df[all_reservoirLocs_df.index.isin(self._userModel().reservoir_IDs)]
            reservoirLocs_X = numpy.array(list(self._userModel().reservoirLocs_df['X']))
            reservoirLocs_Y = numpy.array(list(self._userModel().reservoirLocs_df['Y']))
            print(reservoirLocs_X, type(reservoirLocs_X))
            #print(type(numpy.array(self.reservoir_IDs)[0]))
            self._userModel().ReservoirLocs_map = reservoirs_to_map(self._userModel().streamorder, reservoirLocs_X, reservoirLocs_Y, 0.5, 
                                                       numpy.array(self._userModel().reservoir_IDs), self._userModel().Dir, self._userModel().coords_system)
            ...
            # report(self._userModel().ReservoirLocs_map, os.path.join(self._userModel().Dir, "reservoirs/ReservoirLocs_{}.map".format(runId)))
            # ds = gdal.Open(os.path.join(self._userModel().Dir, "reservoirs/ReservoirLocs_{}.map".format(runId)),GA_Update)
            # ds.SetProjection(self._userModel().coords_system)
            # ds = None
            report(self._userModel().ReservoirLocs_map, os.path.join(self._userModel().SaveDir, "reservoirs/ReservoirLocs_{}.map".format(runId)))
            ds = gdal.Open(os.path.join(self._userModel().SaveDir, "reservoirs/ReservoirLocs_{}.map".format(runId)),GA_Update)
            ds.SetProjection(self._userModel().coords_system)
            ds = None
        ...
        for tsformat in ["csv", "tss"]:
            ...
            while len(toprint) > 0:
                ...
                if "reservoirs/ReservoirLocs.map" in samplemapname:
                    samplemapname = os.path.join(self._userModel().SaveDir, "reservoirs/ReservoirLocs_{}.map".format(runId))
                ...
                if "None" not in samplemapname:
                    ...
                    for a in toprint:
                        if (
                            ...
                            #if a in ["self.ReservoirVolume" ,"self.OutflowSR"] and hasattr(self, "config_reservoir_file") == False:
                            #    continue
    ...
    def wf_saveTimeSeries(self):
        ...
        for a in self.samplenamecsv:
            found = 1
            #if a == ReservoirVolume and hasattr(self, "ReservoirVolume") == False:
            #    continue
            try:
    ...
    def wf_supplyVariableNamesAndRoles(self):
        ...
        res = self.exchangeitems.getvars()
        ...
        if len(res) == 0:
            ...
            for a in API:
                ...
                res.append(tt)
                self.exchangeitems.addvar(tt[0], tt[1], tt[2])
    ...
    def wf_readmap(
        ...
        # find if this is an exchangeitem
        thevars = self.exchangeitems.getvars()
        if len(thevars) == 0:
            self.wf_supplyVariableNamesAndRoles()

        style = self.exchangeitems.getvarStyle(varname)
