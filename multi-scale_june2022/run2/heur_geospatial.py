...
def findFlooded(floodingFeat, reservoirPolyg_shp_filename):
    '''
    returns a list of all the damIDs flooded by floodingFeat
    '''
...
def get_flooded_table(run_folder):
    '''
    reads .csv and returns the DataFrame
    '''
    flooded_table = pd.read_csv(os.path.join(run_folder, "reservoirs", "flooding_table.csv"),
        index_col='damID',
        dtype={'damID':int} 
        )
...
def get_subsets_floodedAreas(subsets_df, resPolyg_df):
    ...
    values are 1 if damID is in subset(column), 0 otherwise
    function calculates the sum of max flooded areas behind dams in each subset
    it returns a floodedArea Series, where each row is a subset, i.e. index = column labels of subsets_df
    '''
    total_cost_dict = {}
    subset_list = subsets_df.columns.tolist()
    for subset_name in subset_list:
...
def create_special_subsets(reservoirPolyg_shp_filename, run_name):
    '''
    ...
    '''
    run_folder = os.path.join(current_folder, run_name)
    
    if not os.path.exists(os.path.join(run_folder,"special_subsets.csv")):
        resPolyg_df = gpd.read_file(os.path.join(run_folder, "reservoirs", reservoirPolyg_shp_filename))
        resPolyg_df = resPolyg_df.set_index(["streamID", "damID"], drop=False)
        resPolyg_df.rename(columns={"CChaul_tot": "cost"}, inplace=True)

        if "flooding_table.csv" in os.listdir((os.path.join(run_folder, "reservoirs"))):
            flooded_table = get_flooded_table(run_folder)
        else:
            flooded_table = build_flooded_table(os.path.join(run_folder, "reservoirs", reservoirPolyg_shp_filename))
            flooded_table.to_csv(os.path.join(run_folder, "reservoirs", "flooding_table.csv"),
                index_label = "damID"
                )

        resPolyg_df = resPolyg_df.loc[resPolyg_df.loc[:,"region"]=='downstream', :]

        resPolyg_df["cost_to_VOL"] = resPolyg_df["cost"]/resPolyg_df["VOLmax"]
        resPolyg_df["orif_to_inflow"] = resPolyg_df["B_orif"]*resPolyg_df["H_orif"]/resPolyg_df["INFLOW_VOL"]
        ...
        special_df.to_csv(os.path.join(run_folder,"special_subsets.csv"))
    else:
        print("special_df esiste gia!")
        special_df = pd.read_csv(os.path.join(run_folder,"special_subsets.csv"))
...
# the following is not executed when heur_geospatial module is imported by another module
# only executed when this module is run (e.g., from command line)
if __name__ == "__main__": 
    current_folder = os.getcwd()
    run_name = "hewett_template"
    reservoirPolyg_shp_filename = "reservoirPolygon2.0_by_Vmax_to_damLength_design_cost.shp"
    # special_df = create_special_subsets(reservoirPolyg_shp_filename, run_name)
    # find_duplicates_in_special(special_df, run_name)
