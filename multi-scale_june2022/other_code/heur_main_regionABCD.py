...
comb_gen_df, comb_values_df = merge_optimal_solutions(regional_runs, 20, num_subsets, current_folder)
...
new_subsetIDs = [get_name_from_num(i, 20) for i in range(len(subsetIDs))]
...
resorted_comb_values_df = h_gen.rankSort_curr_gen(comb_values_df, 20, run_name)
resorted_comb_values_df.to_csv(os.path.join(runFolder, "ABCD_gen20_values_resorted.csv"), index_label = "subsetID")
ranked_subsetID_list = resorted_comb_values_df.index.tolist()
comb_gen_df = comb_gen_df.loc[:, ranked_subsetID_list]
comb_gen_df.to_csv(os.path.join(runFolder, "gen20_subsets.csv"), index_label = "damID")
'''
...
# # reindexed_curr_generation_df.fillna(0, inplace=True)
# reindexed_curr_generation_df.to_csv((os.path.join(runFolder, "gen{}_subsets.csv".format(gen_num))))



for gen_num in range(161, num_generations):
