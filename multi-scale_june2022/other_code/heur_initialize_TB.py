...
def init_gen0(resPolyg_df, num_subsets, runFolder, max_strahl=5): # budget, maxFloodedArea, 
    ...
    budget, maxFloodedArea: constraints
    ...
    # cost_dict = {}
    # floodedArea_dict = {}
    for subset_name in colnames:
        # cost = 0
        # floodedArea = 0
        available = list(damIDList)
        selected_damIDs = []
        max_to_select = math.ceil(len(available)/5)
        min_to_select = max(2,math.floor(len(available)/200))
        
        num_of_reservoirs = random.sample(range(min_to_select,max_to_select),1)[0]  # random num of reservoirs in each subset, min 5, max 10% of all locations
        count_of_reservoirs = 0
        max_number_of_attempts = 10
        attempt = 1
        # while (cost < budget and floodedArea < maxFloodedArea and attempt <= number_of_attempts ):
        while (count_of_reservoirs < num_of_reservoirs and attempt <= max_number_of_attempts):
