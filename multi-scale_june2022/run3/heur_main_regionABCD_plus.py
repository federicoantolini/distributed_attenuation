...
def merge_optimal_solutions(regional_runs, last_optimal_gen_num, num_subsets, current_folder):
    ...
    for i in range(len(regional_runs)):
        ...
        last_gen_df.rename(columns=lambda x:regional_run_name[-1]+x, inplace=True)
        ...
        last_values_df.rename(index=lambda x:regional_run_name[-1]+x, inplace=True)
    ...
    comb_values_df.sort_values(by=["rank","distance"], ascending=[True, False], inplace=True)
    ...
    comb_gen_df.sort_index(axis=0, inplace=True)
...
'''
regional_runs = ["regionA", "regionB", "regionC", "regionD"]
comb_gen_df, comb_values_df = merge_optimal_solutions(regional_runs, gen_num, num_subsets, current_folder)
comb_gen_df.to_csv(os.path.join(runFolder, "ABCD_gen20_subsets.csv"), index_label = "damID")
comb_values_df.to_csv(os.path.join(runFolder, "ABCD_gen20_values.csv"), index_label = "subsetID")
subsetIDs = comb_values_df.index.tolist()
new_subsetIDs = [get_name_from_num(i, gen_num) for i in range(len(subsetIDs))]
...
comb_gen_df.to_csv(os.path.join(runFolder, "ABCD_gen20_subsets_renamed.csv"), index_label = "damID")
comb_values_df.to_csv(os.path.join(runFolder, "ABCD_gen20_values_renamed.csv"), index_label = "subsetID")

next_gen_num = gen_num+1
comb_gen_plus_df = h_gen.create_next_generation_after_combination(comb_gen_df, comb_values_df, gen_num, resPolyg_df, num_subsets, runFolder)
comb_gen_plus_df.to_csv(os.path.join(runFolder, "gen{}_subsets.csv".format(next_gen_num)), index_label = "damID")
'''
'''
next_gen_num = gen_num+1
comb_gen_plus_df = pd.read_csv(os.path.join(runFolder, "gen{}_subsets.csv".format(next_gen_num)), index_col="damID")
h_paral.job_setup_and_launch(comb_gen_plus_df, next_gen_num, runFolder, lowercase_watershed, timesteps, queue, core_num, 2*num_subsets)
h_rem.remove_useless_files(run_name, next_gen_num)
summarize_simulation_results(comb_gen_plus_df, resPolyg_df, Q0, runFolder, next_gen_num)
# h_geo.subsets_to_SHP(gen0_df, resPolyg_df, runFolder)
print("\n\nGeneration {} concluded!\n\n".format(next_gen_num))
'''

# curr_generation_df = pd.read_csv(os.path.join(runFolder, "gen{}_subsets.csv".format(gen_num)), index_col="damID")
# reindexed_curr_generation_df = curr_generation_df.reindex(index=resPolyg_df.index, fill_value=0)
# reindexed_curr_generation_df.to_csv((os.path.join(runFolder, "gen{}_subsets.csv".format(gen_num))))


for gen_num in range(151, num_generations):
