...
def get_curr_generation_subsets(gen_num, run_name):
    ...
    each row is a damID 
    columns are subsets
    values are 1 if damID is in subset, 0 otherwise
    '''
    ...
    generation_subsets_df = pd.read_csv(generation_csv_filename, \
        # names=["damID"]+next_p arents_list, 
        index_col="damID", 
...
def check_feasible(subset, new_damID_list, runFolder):
    '''
    checks that subset is feasible:
...
def save_ranked_values(p_c_pooled_df, values_df, run_name, gen_num):
    '''
    exports current gen values, rank and dist to a .csv
    '''
...
def rankSort_curr_gen(values_df, gen_num, run_name):
    '''
    ...
    cost_max_threshold = 5000000
    p_c_pooled_df = p_c_pooled_df[p_c_pooled_df["cost"] < cost_max_threshold]
    qpeak_reduction_min_threshold = 0.1 * p_c_pooled_df["Qpeak_reduction"].max()
    p_c_pooled_df = p_c_pooled_df[p_c_pooled_df["Qpeak_reduction"] > qpeak_reduction_min_threshold]
    ...
    print(p_c_pooled_df)
    save_ranked_values(p_c_pooled_df, values_df, run_name, gen_num)
    
    return p_c_pooled_df



def select_next_parent_values(rankSorted_curr_gen_df, retention_rate=0.2):
    '''
    passes the best solutions to next gen
    '''
    num_of_solutions = rankSorted_curr_gen_df.shape[0]
    return rankSorted_curr_gen_df.iloc[:int(num_of_solutions*retention_rate), :]
...
def print_rank_dist(values_df, gen_num, run_name):
    '''
    function to assign rank and distance to the final generation, to identify the last Pareto frontier
    rank by domination and sort by crowding distance
    '''
    # assign ranks to solutions based on domination relationships
...
def mutation(series, runFolder, n=0):
    '''
    series is a Series, a solution/subset to mutate to generate a child/mutated_ser
    n is the number of mutations to apply to the series
    function returns the mutated_ser, a feasible subset of type Series 
    '''
    nrows = series.shape[0]
    mutated_ser = series.copy(deep=True)
    ...
    while k<=n:
        ...
        mutation_damID = series.iloc[mutation_cand_index:].index.tolist()[0]
        if mutated_ser.loc[mutation_damID] == 1:
            mutated_ser.loc[mutation_damID] = 0
            if mutated_ser.any()==False:
                mutated_ser.loc[mutation_damID] = 1
                continue
            k+=1
        else:
            mutated_ser.loc[mutation_damID] = 1
            # check that the new reservoir is not flooded by nor does flood one of the other reservoirs
            if check_feasible(mutated_ser, [mutation_damID], runFolder) == True:
                k+=1
            else:
                mutated_ser.loc[mutation_damID] = 0
...
def create_weights(rankSorted_curr_gen_df):
    '''
    assigns a weight to a subset based on its rank, the lower the rank, the higher the weight
    '''
    weights = 1/(rankSorted_curr_gen_df["rank"]+1)
    print(weights)
    return weights
    
    

def create_children_with_crossover(curr_generation_subsets_df, weights_ser, num_of_crossover_operations, resPolyg_df, gen_num, runFolder):
    '''
    applies crossover_swapOnStream and creates children from curr_generation_subsets_df
    How many children? as many as num_of_crossover_operations
    The selection of parents uses weights, meaning that the lower the rank, the higher the possibility of being randonly chosen as parents
    Returns children, a DataFrame, children are columns
    
    '''
    children = pd.DataFrame(0, index=curr_generation_subsets_df.index, columns=["to_remove"])
    # apply crossover to random pairs of parents and create some children
    crossover_j = 1
    while crossover_j <= num_of_crossover_operations:
        selected_parents = curr_generation_subsets_df.sample(2, replace=False, weights=weights_ser, axis='columns')
        # parent1_index = np.random.randint(0,num_of_parents)
        # parent2_index = np.random.randint(0,num_of_parents)
        # while parent2_index == parent1_index:
        #     parent2_index = np.random.randint(0,num_of_parents)
        # selected_parents = curr_generation_subsets_df.iloc[:, [parent1_index, parent2_index]]
        crossover_children = crossover_swapOnStream(selected_parents, resPolyg_df, runFolder)
        gc_colnames = crossover_children.columns.tolist()
        new_colnames = {gc_colnames[0]:get_name_from_num(2*crossover_j-2, gen_num),
        gc_colnames[1]:get_name_from_num(2*crossover_j-1, gen_num)}
...
def mutate_children(children, num_of_mutation_operations, resPolyg_df, gen_num, runFolder):
    '''
    applies mutation to previously created children (created with crossover)
    '''
    mutated_children = children.copy(deep=True)
    for j in range(children.shape[1]):
        subset_name = children.iloc[:, j].name
        mutated_children[subset_name] = mutation(children[subset_name], runFolder)

    return mutated_children



def create_children3(curr_generation_subsets_df, weights_ser, num_of_next_gen_parents, num_subsets, resPolyg_df, gen_num, runFolder):
    '''
    parents: DataFrame of subsets, one in each column, generated by initialization or by nsga2 (create_next_parent)
    1. applies crossover (swapOnStream) to generate children DataFrame
    2. applies mutation to all these children
    Hence all children are the result of: first crossover, then mutation
    children contains as many subsets as parents
    both parents and children are run on wflow with function create_job and run_job in module heur_parallelize
    '''

    num_of_parents = curr_generation_subsets_df.shape[1]
    num_of_children = 2*num_subsets - num_of_next_gen_parents
    num_of_crossover_operations = num_of_children/2
    num_of_mutation_operations = num_of_children
    # print("curr_gen: {}".format(num_of_parents), "next_gen_parents: {}".format(num_of_next_gen_parents) ,"children: {}".format(num_of_children))
    # digits = len(str(2*num_of_parents))

    children = create_children_with_crossover(curr_generation_subsets_df, weights_ser, num_of_crossover_operations, resPolyg_df, gen_num, runFolder)
    print("done with crossover")
    # print("curr_gen: {}".format(num_of_parents), "next_gen_parents: {}".format(num_of_next_gen_parents) ,"children: {}".format(children.shape[1]))
    children = mutate_children(children, num_of_mutation_operations, resPolyg_df, gen_num, runFolder)
...
def get_simulation_results(gen_num, run_name):
    '''
    reads results of simulation, in which parents and children are run with wflow, parallelized on HPC
    '''
    current_folder = os.getcwd()
    results_csv_filename = os.path.join(current_folder, run_name, "gen{}_qpeak_cost.csv".format(gen_num))
    results_df = pd.read_csv(results_csv_filename, index_col="subsetID")#, usecols=["Qpeak_reduction", "cost"])
    return results_df



def create_next_generation(gen_num, resPolyg_df, run_name, num_subsets, runFolder):
    '''
    ...
    parents and children are rows, index in the first column; "Qpeak_reduction" and "cost" are columns
    ...
    function creates parents of next generation
    nextgen parents generate nextgen children, with crossover and mutation
    dfs concatenated together into nextgen df
    check for duplicate subsets columns, in case drop duplicate child(ren) and add new child(ren)
    returns nextgen df
    '''
    ...
    rankSorted_curr_gen_df = rankSort_curr_gen(values_df, gen_num, run_name) # NonDom, DistSorted ranking
    # rankSorted_curr_gen_df = pd.read_csv(os.path.join(runFolder, "gen{}_qpeak_cost_rank.csv".format(gen_num)), index_col="subsetID")
    rankSorted_curr_gen_list = rankSorted_curr_gen_df.index.tolist()

    next_parents_values_df = select_next_parent_values(rankSorted_curr_gen_df, retention_rate=0.2)
    next_parents_list = next_parents_values_df.index.tolist()
    num_of_next_gen_parents = len(next_parents_list)

    curr_generation_subsets_df = get_curr_generation_subsets(gen_num, run_name)
    curr_generation_subsets_df = curr_generation_subsets_df.loc[:, rankSorted_curr_gen_list]
    weights_ser = create_weights(rankSorted_curr_gen_df)
    ...
    next_children_subsets_df = create_children3(curr_generation_subsets_df, weights_ser, num_of_next_gen_parents, num_subsets, resPolyg_df, next_gen_num, runFolder)
    next_generation_subsets_df = pd.concat([next_parents_subsets_df, next_children_subsets_df], axis=1)
    print("next_gen: {}".format(next_generation_subsets_df.shape[1]), 
    "next_gen_parents: {}".format(next_parents_subsets_df.shape[1]) ,
    "next_gen_children: {}".format(next_children_subsets_df.shape[1]))
    ...
    while possible_duplicates == True:
        ...
        if after_dropdupli_df.shape == next_generation_subsets_df.shape:
            ...
            next_generation_subsets_df = add_children(next_parents_subsets_df, after_dropdupli_df.iloc[:, num_of_next_gen_parents:], num_subsets, next_gen_num, runFolder)
            possible_duplicates = True
    return next_generation_subsets_df



def create_next_generation_after_combination(curr_generation_subsets_df, rankSorted_curr_gen_df, gen_num, resPolyg_df, num_subsets, runFolder):
    '''
    used after combining optimal solutions from different regions
    This function is essentially the same as create_next_generation
    Here rankSorted_curr_gen_df is created manually by the researcher, who combines the results form independent optimizations in different regions
    Researcher may desire equal or fair representation from all regions, even when optimal solutions from one region are better than optimal in another
    
    returns nextgen df, DataFrame of parents and children that starts the new cycle of optimization
    '''
    
    rankSorted_curr_gen_list = rankSorted_curr_gen_df.index.tolist()
    
    next_parents_values_df = select_next_parent_values(rankSorted_curr_gen_df, retention_rate=0.3)
    next_parents_list = next_parents_values_df.index.tolist()
    num_of_next_gen_parents = len(next_parents_list)
