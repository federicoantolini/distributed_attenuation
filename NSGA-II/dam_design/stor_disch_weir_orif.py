...
def calc_Q_weir_rect_orif(depth, B_orif, Hb, g):
    '''
    treating the unsubmerged upstream orifice as a broad_crested weir
    critical h above the weir is 2/3*H, H is water height measured from orifice bottom edge
    for a rectangular weir the formula for Q is simple
    
    Borif: orifice width
    Hb: the elevation of orifice bottom edge from streambed
    g: gravity constant
    '''
...
def calc_Q_weir_circ_orif(depth, R, Hb, g, num_pipes, integr_step):
    ...
    for a circular weir, the formula is NOT simple 
...
def calc_Q_above_rect_orif(depth, B_orif, H_orif, Hb, cd, g):
    '''
    calculate Q through a rectangular orifice, given water depth in the dam
    '''
    Q = (2/3) * cd * math.sqrt(2*g) * B_orif * ((depth-Hb)**1.5 - (depth-Hb-H_orif)**1.5)
    return Q
...
def build_storage_array(depth_vol_series):
    '''
    create an array of storage values, one for each depth provided in depth_vol_series
    in depth_vol_series, depth is the index, vol is the value
    use interpolate function to find storage for depth values for which storage is not already available 
    '''
...
def build_depth_array(maxh, step):
    '''
    create an array of depths from 0 to maxh
    step: distance between consecutive values
    '''
...
def build_depth_series(maxh_series, step):
    '''
    given a series of maxh (one maxh for each damID), apply function build_depth_array
    returns a series of depth arrays (one array for each maxh/damID)
    '''
...
def build_storage_series(df, depth_series):
    '''
    returns a series of storage/vol arrays (one array for each maxh/damID)
    '''
...
#     longest_depth_list = list(longest_depth_array)[:-1] + ['max']
    ...
    # #return depth_vol_df
    storage_series = build_storage_array(depth_vol_df)
...
reservoir_data_dbf = "reservoirPolygon2.0_by_Vmax_to_damLength_design_cost.dbf"
stor_outfl_folder = os.path.join("stor_outfl_files") 
if os.path.exists(stor_outfl_folder) == False:
    os.mkdir(stor_outfl_folder)
reservoirs_folder = os.path.join("reservoirs")
huc10 = 'Volga'
huc12 = 'Hewett'
step = 0.10
integr_step = 0.01
...
res_df = gpd.read_file(os.path.join(reservoirs_folder, reservoir_data_dbf))
...
res_df["R"] = res_df["D_orif_met"]/2

depStoOutf_all_df = pd.DataFrame(columns = ['depth','storage', 'outflow_rect', 'damID'])  # can add 'outflow_circ' to the columns

for damID in res_df.index.tolist():
# # for damID in [601667]:
#     # circular orifice
#     R = res_df.loc[damID, "R"]
#     num_pipes = res_df.loc[damID, "num_pipes"]
#     # print(R)
#     htop = round(Hb + 2*R, 2)
...
#     # # print(circ_orif_concat_Q_series)
    ...
    rect_orif_depth_array = np.round(np.arange(0, Hb+H_orif*1.5-0.000001, step), 1)
    
    rect_orif_depth_series = pd.Series(rect_orif_depth_array, index=rect_orif_depth_array)
    rect_orif_Q_series = rect_orif_depth_series.apply(calc_Q_weir_rect_orif, args=[B_orif, Hb, g])
    ...
    above_rect_orif_Q_series = above_rect_orif_depth_series.apply(calc_Q_above_rect_orif, args=[B_orif, H_orif, Hb, cd, g])
    # print(above_rect_orif_Q_series)
    rect_orif_concat_Q_series = rect_orif_Q_series.append(above_rect_orif_Q_series)
    
    stor_out_df = pd.read_table(os.path.join(reservoirs_folder, "{}_df.txt".format(damID)), index_col='depth')
    stor_out_df.drop(columns='Unnamed: 0', inplace=True)
    # storage_array = stor_out_df['storage'].array
    # storage_series = pd.Series(storage_array, index=for_storage_depth_array)
    for_storage_depth_array = stor_out_df.index.array
    for_storage_depth_series = pd.Series(for_storage_depth_array, index=for_storage_depth_array)
    storage_series = stor_out_df['storage']

    # for_storage_depth_array = build_depth_array(res_df.loc[damID, "maxH"], step)
    # for_storage_depth_series = pd.Series(for_storage_depth_array, index=for_storage_depth_series)
    ...
    try:
        ...
        'storage':storage_series.round(decimals=2), 
        'outflow_rect': rect_orif_concat_Q_series.round(decimals=2),
        # 'outflow_circ': circ_orif_concat_Q_series
        })
    except:
        print(damID)
        break
    depStoOutf_df.to_csv(os.path.join(stor_outfl_folder, "{}_df.txt".format(damID)), sep='\t')
...
depStoOutf_all_df.to_csv(os.path.join(stor_outfl_folder, "all_reservoirs_df.txt"), sep='\t')
    


# res_df['rect_Chezy_diff'] = pd.Series(rect_Chezy_diff_dict)
# res_df.to_file(os.path.join("reservoirs", stor_disch_folder, "reservoirPolygon2.0_by_Vmax_to_damLength_rect_circ.shp"), index=False)