...
def plot_single_generation(df, runID, run_name, gen_num, runFolder, decimation=False):
    ...
    title = "Generation {}".format(gen_num[:3])
    ttl = ax.title
    ...
    notes = ax.text(0.025, 0.96, notes_str, transform=ax.transAxes, fontsize=13,
    ...
    #set legend
    leg = ax.legend(loc=(0.015, 0.53), fontsize=13, shadow=False) #'upper left'
    ...
    # if decimation==True:
    #     fig_filepath = os.path.join(runFolder,"images", "_gen{}d.png".format(gen_num))
...
def plot_multi_gen_Pareto(runID, run_name, runFolder, gen_num_list='all'):
    ...
    Pareto_df = pd.read_csv(os.path.join(runFolder, "Pareto_{}.csv".format(run_name)))
    ...
    notes = ax.text(0.22, 0.96, notes_str, transform=ax.transAxes, fontsize=12,
                    verticalalignment='top')
    notes.set_bbox(dict(boxstyle='round', facecolor='whitesmoke', edgecolor="darkgrey"))

    #set legend
    leg = ax.legend(loc=(0.01, 0.30), fontsize=12, shadow=False)
    leg.get_frame().set_facecolor('whitesmoke')
...
def plot_generation_breakdown(runFolder, run_name, gen_n=100):
