...
# Initialiaze parents of generation zero
parents0_df = h_init.init_gen0(resPolyg_df, num_subsets, runFolder, max_strahl)
...
children0_df = h_gen.create_children2(parents0_df, resPolyg_df, gen_num, runFolder, crossover_prop)
...
# run subsets and summarize results
gen0_df = pd.read_csv(os.path.join(runFolder, "gen{}_subsets.csv".format(gen_num)), index_col="damID")
h_paral.job_setup_and_launch(gen0_df, gen_num, runFolder, lowercase_watershed, timesteps, queue, core_num, num_subsets)
...
# here starts optimization cycle
for gen_num in range(103, num_generations):
    # apply decimation once every 5 generations 
    if gen_num%5 == 1:
        curr_generation_df = h_gen.create_decimated_generation(gen_num-1, resPolyg_df, run_name, num_subsets, runFolder, crossover_prop, decimation_survival_rate)
    else:
        curr_generation_df = h_gen.create_next_generation(gen_num-1, resPolyg_df, run_name, num_subsets, runFolder, crossover_prop)
