...
def plot_single_Pareto_subset(Pareto_df, runID, run_name, gen_num, subsetID, runFolder):
    ...
    ax.set_xlim(xmin=-1, xmax=35)
    ax.set_xticks([i for i in range(36) if i%5==0])
    ax.set_xticklabels([i for i in range(36) if i%5==0], fontsize=13)
...
def plot_multi_gen_Pareto(runID, run_name, runFolder, gen_num_list='all'):
    Pareto_filename_list = [fn for fn in os.listdir(runFolder) if fn.startswith("Pareto_{}".format(run_name)) ]
    print(Pareto_filename_list)
    Pareto_df = pd.read_csv(os.path.join(runFolder, Pareto_filename_list[0]) )
    ...
    #set legend
    leg = ax.legend(loc=(0.02, 0.32), fontsize=12, shadow=False)
    leg.get_frame().set_facecolor('whitesmoke')
...
def create_label_from_run_name(run_name):
    run_name_components = run_name.split('_')
    region_dict = {"up":"upstream", "mid":"midstream", "down":"downstream"}
    region = region_dict[run_name_components[0]]
    crossover_proportion = run_name_components[1][:2]
    mutation_proportion = run_name_components[1][2:4]
    special_subsets_add = ""
    if "SP" in run_name_components[1]:
        special_subsets_add = ", SPECIAL S."
    label = "{}% C., {}% M.".format(crossover_proportion, mutation_proportion) + special_subsets_add
    return label
        


def plot_diversity_within(run_name_list, current_folder):
    
    fig, ax = plt.subplots(figsize=[9, 5.2], facecolor='whitesmoke')
    ax.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))

    title = "Diversity rate"
    ttl = ax.title
    ttl.set_position([0.5,1.00])
    ax.set_title(title, fontsize=18, y=1.03)

    ax.set_xlim(xmin=-1, xmax=101)
    ax.set_xticks([i for i in range(101) if i%10==0])
    ax.set_xticklabels([i for i in range(101) if i%10==0], fontsize=13)
    
    ax.set_ylim(ymin=0.009, ymax=0.051)
    ax.set_yticks([i/1000 for i in range(10,51) if i%5==0])
    ax.set_yticklabels([i/1000 for i in range(10,51) if i%5==0], fontsize=13)

    ax.grid(color='black', axis='both', alpha=0.2)
    ax.set_xlabel('Generation number', size=16, labelpad=7)
    ax.set_ylabel("Diversity rate", size=16, y=0.5, labelpad=7)

    colors = ["dodgerblue", "firebrick", "limegreen", "deeppink"]
    colors = [
    ...
    for i in range(len(run_name_list)):
        ...
        run_name = run_name_list[i]
        legend_label = create_label_from_run_name(run_name)
        runFolder = os.path.join(current_folder, run_name)
        Pareto_diversity_within_filepath = os.path.join(runFolder, "Pareto_diversity_within_{}_{}-{}.csv".format(run_name, 0, 100))
        Pareto_diversity_within_df = pd.read_csv(Pareto_diversity_within_filepath, index_col='gen_num')
        ax.plot(Pareto_diversity_within_df.index, Pareto_diversity_within_df["diversity_within"], 
                    linestyle=linestyles[math.floor(i/2)],
                    #linestyle='-', 
                    marker=None, 
                    # color = 'k', 
                    color=colors[i%2], 
                    linewidth=1.5, markersize=4, alpha=1.0, # 0.3
                    # label='_'+run_name,
                    label = legend_label,
                    )
    ...
    plt.gcf().subplots_adjust(top=0.89, bottom=0.15, right=0.96, left=0.12)

    image_filename = "diversity_rate" # 2
    image_filepath = os.path.join(current_folder, "{}.png".format(image_filename))
    fig.savefig(image_filepath,
                facecolor='white', edgecolor='black')
    add_border(image_filepath)
...
def plot_breakdown_by_gen(runFolder, runID, run_name, gen_n=100):
